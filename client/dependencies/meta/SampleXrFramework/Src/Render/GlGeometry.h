...
// Use 32-bit indices by default to allow large meshes. Many places use
// `TriangleIndex` typedef and GL will use GL_UNSIGNED_INT when sizeof==4.
...
class GlGeometry {
   ...
   public:
    static constexpr int32_t MAX_GEOMETRY_VERTICES = std::numeric_limits<int32_t>::max() / 12;
    static constexpr int32_t MAX_GEOMETRY_INDICES = std::numeric_limits<int32_t>::max();
