...
bool GlGeometry::enableGeometryTransfom = false;
OVR::Matrix4f GlGeometry::geometryTransfom = OVR::Matrix4f();
...
GlGeometry::Descriptor BuildTesselatedQuadDescriptor(
    ...
    // number of quads is (horizontal x vertical)
    // so vertices are (horizontal+1)*(vertical+1)
    const int vertsX = horizontal + 1;
    const int vertsY = vertical + 1;
    ...
    // reserve index buffer capacity: 2 triangles per quad -> 6 indices per quad
    const size_t quads = (size_t)horizontal * (size_t)vertical;
    const size_t faces = quads * 2u;
    const size_t totalIndices = faces * 3u * (twoSided ? 2u : 1u); // double if twoSided
    std::vector<TriangleIndex> indices;
    indices.reserve(totalIndices);

    // helper lambda to push a triangle (casts to TriangleIndex)
    auto pushTri = [&](int a, int b, int c) {
        indices.push_back(static_cast<TriangleIndex>(a));
        indices.push_back(static_cast<TriangleIndex>(b));
        indices.push_back(static_cast<TriangleIndex>(c));
    };

    // build triangles for each quad
    for (TriangleIndex y = 0; y < vertical; ++y) {
        for (TriangleIndex x = 0; x < horizontal; ++x) {
            const int tl = y * vertsX + x;
            const int tr = y * vertsX + (x + 1);
            const int bl = (y + 1) * vertsX + x;
            const int br = (y + 1) * vertsX + (x + 1);

            // two triangles per quad (consistent winding: CCW)
            pushTri(tl, tr, bl); // top-left, top-right, bottom-left
            pushTri(bl, tr, br); // bottom-left, top-right, bottom-right
        }
    }

    // if twoSided, append back-facing triangles with reversed winding
    if (twoSided) {
        const size_t frontCount = indices.size();
        // iterate existing triangles and append reversed order
        for (size_t i = 0; i < frontCount; i += 3) {
            TriangleIndex a = indices[i + 0];
            TriangleIndex b = indices[i + 1];
            TriangleIndex c = indices[i + 2];
            // reversed winding
            indices.push_back(c);
            indices.push_back(b);
            indices.push_back(a);
...
GlGeometry::Descriptor BuildTesselatedCylinderPatchDescriptor(
    ...
    return {attribs, indices, GlGeometry::geometryTransfom};
}
...
BuildDomeDescriptor(const float latRads, const float uScale, const float vScale) {
...
GlGeometry::Descriptor BuildGlobeDescriptor(
    ...
    for (int y = 0; y <= vertical; y++) {
        ...
        if (y <= poleVertical) {
            ...
            yf = (float)(uniformVertical - 1 +
                         ((float)(y - (vertical - poleVertical - 1)) / (poleVertical + 1))) /
                uniformVertical;
        } else {
...
GlGeometry::Descriptor BuildWedgeDescriptor(
    ...
    for (TriangleIndex index = 0; index <= divisions; ++index) {
        const int edge = 2 + index * 4;
        const float angle = angleStart + (((angleStop - angleStart) / divisions) * index);
        // build wedge in the X/Y plane
        attribs.position[edge + 0].x = attribs.position[edge + 1].x = attribs.position[edge + 2].x =
            attribs.position[edge + 3].x = cosf(angle) * radius;
        attribs.position[edge + 0].y = attribs.position[edge + 1].y = attribs.position[edge + 2].y =
            attribs.position[edge + 3].y = sinf(angle) * radius;
        ...
        attribs.normal[edge + 1] = attribs.normal[edge + 2] =
            OVR::Vector3f(attribs.position[edge + 1].x, attribs.position[edge + 1].y, 0)
                .Normalized();
        ...
        if (sides && (index == 0 || index == divisions)) {
            ...
            attribs.position[edgeSides + 1].x = attribs.position[edgeSides + 2].x =
                attribs.position[edge + 0].x;

            attribs.position[edgeSides + 0].y = attribs.position[edgeSides + 3].y = 0.0f;
            attribs.position[edgeSides + 1].y = attribs.position[edgeSides + 2].y =
                attribs.position[edge + 0].y;

            attribs.position[edgeSides + 0].z = attribs.position[edgeSides + 1].z =
                attribs.position[edge + 0].z;
            attribs.position[edgeSides + 2].z = attribs.position[edgeSides + 3].z =
                attribs.position[edge + 2].z;
